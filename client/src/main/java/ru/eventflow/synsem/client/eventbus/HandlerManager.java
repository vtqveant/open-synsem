 ...
 * the License.
 */
package ru.eventflow.synsem.client.eventbus;
...
public class HandlerManager {
     ...
     */
    Map<Event.Type<?>, ArrayList<?>> createHandlerInfo() {
        return registry.map;
    }
    ...
    private <H extends EventHandler> void doAdd(Event.Type<H> type, final H handler) {
        registry.addHandler(type, handler);
    }

    private <H extends EventHandler> void doRemove(Event.Type<H> type, final H handler) {
        registry.removeHandler(type, handler);
    }

    private <H extends EventHandler> void enqueueAdd(final Event.Type<H> type, final H handler) {
    ...
    private <H extends EventHandler> void enqueueRemove(final Event.Type<H> type, final H handler) {
        defer(new AddOrRemoveCommand() {
            public void execute() {
                doRemove(type, handler);
            }
    ...
    private static class HandlerRegistry {
        ...
        private <H extends EventHandler> void addHandler(Event.Type<H> type, H handler) {
        ...
        private <H extends EventHandler> void fireEvent(Event<H> event, boolean isReverseOrder) {
            Event.Type<H> type = event.getAssociatedType();
